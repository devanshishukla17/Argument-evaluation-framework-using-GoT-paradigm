"""
app.py — Main Streamlit entry point for EssayEvalAI.
...
from got_engine import generate_branches, rank_branches, merge_branches, generate_final_verdict
...
from visualization import (argument_network_chart, got_branch_timeline, branch_score_radar,
...
from export_utils import export_csv, export_json, export_pdf_report
...
with st.sidebar:
    ...
    pages = [("🏠","Home Dashboard"),("🗺","Full Argument Graph"),("🧠","GoT Explorer"),
             ...
             ("🎓","Student Dashboard"),("👩‍🏫","Teacher Dashboard"),("📤","Export Report")]
...
def _run_analysis(essay_text: str):
    ...
    with st.spinner("🧠 Generating Graph-of-Thought branches…"):
        branches = generate_branches(df)
    ...
    with st.spinner("📝 Generating feedback…"):
        verdict = generate_final_verdict(ranked, stats)
...
def _render_input():
    ...
    with col_input:
        method = st.radio("Input", ["✍️ Type / Paste","📁 Upload File"], horizontal=True, label_visibility="collapsed")
        essay_text = ""
        if "✍️" in method:
    ...
    if not st.session_state.analysed:
        ...
        cards = [("🗺","Argument Graph","Visualise how claims, evidence and counterarguments connect."),
                 ...
                 ("📤","Export Report","Download full analysis as CSV, JSON or PDF.")]
        cols = st.columns(3)
...
# ─── FULL ARGUMENT GRAPH ─────────────────────────────────────────────────────
    ...
    if not st.session_state.analysed:
        ...
        with col2:
            fig3 = argument_network_chart(build_hidden_assumption_graph(st.session_state.df), "Hidden Assumption Graph")
            st.plotly_chart(fig3, use_container_width=True)
            st.markdown(graph_insight_html(
                "Hidden Assumption view: orange nodes flag unstated premises that claims silently depend on. "
                "If a claim has no citation, a hidden assumption node is added."
            ), unsafe_allow_html=True)
...
# ─── GoT EXPLORER ─────────────────────────────────────────────────────────────
    ...
    if not st.session_state.analysed:
        ...
        st.markdown(graph_insight_html(
            "Radar chart comparing all branches across Confidence, Completeness, and Diversity. "
            "The largest polygon is the strongest branch. Ideal branches cover all four dimensions evenly."
        ), unsafe_allow_html=True)
...
# ─── DEVIL'S ADVOCATE ─────────────────────────────────────────────────────────
    ...
    if not st.session_state.analysed:
        ...
        for pt in syn.get("summary_points",[]):
            cls = "feedback-weakness" if ("🚨" in pt or "⚠️" in pt) else "feedback-strength"
        ...
        agent_meta = {
            "logical":        ("⚖️","Logical Devil","Checks whether your reasoning makes sense. Looks for logical fallacies (e.g. 'everyone knows'), contradictions between statements, and claims you made without any proof."),
            ...
            "counterargument":("⚔️","Counterargument Devil","For each of your main claims, generates the strongest possible opposing argument. Use these to strengthen your essay by pre-empting objections."),
        ...
        for key in ["logical","ethical","domain","cultural","counterargument","evidence","pedagogy"]:
            ...
            with st.expander(f"{ico} {name} — {agent.get('count',0)} finding(s)"):
                ...
                for f in findings[:12]:
                    ...
                    sent_html = f'<div style="font-size:.82rem;color:#9298ae;margin:.3rem 0;font-style:italic">"{f["sentence"][:100]}{"…" if len(f["sentence"])>100 else ""}"</div>' if f.get("sentence") else ""
                    counter_html = f'<div style="font-size:.82rem;color:#4ecdc4;margin-top:.4rem">💬 Strongest counter: {f["counter"]}</div>' if f.get("counter") else ""
                    st.markdown(f"""<div class="feedback-item {cls}">
                        <div style="font-size:.72rem;font-family:'DM Mono',monospace;opacity:.7;margin-bottom:.3rem">{f.get('type','')} · {f.get('subtype','')}</div>
                        {sent_html}
                        {f.get('student_note','')}
                        {counter_html}
                    </div>""", unsafe_allow_html=True)
...
# ─── EXPORT REPORT ────────────────────────────────────────────────────────────
elif page_name == "Export Report":
    st.markdown("""<div class="page-header">
        <div class="page-title">Export Report</div>
        <div class="page-subtitle">CSV · JSON · PDF — Save & Share Your Analysis</div>
    </div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.analysed:
        ...
        # What is this page?
        st.markdown("""<div class="graph-insight">
            📤 <b>Why export?</b> The Export page lets you save the full analysis as a file so you can 
            share it with a teacher, submit it alongside your essay, review it offline, or import it 
            into Excel or Google Sheets for further analysis. Choose the format that suits your purpose.
        </div>""", unsafe_allow_html=True)

        # Format explainer cards
        c1,c2,c3 = st.columns(3)
        with c1:
            st.markdown("""<div class="section-card" style="text-align:center;min-height:160px">
                <div style="font-size:2rem;margin-bottom:.5rem">📋</div>
                <div style="font-family:'Syne',sans-serif;font-weight:700;margin-bottom:.4rem">CSV</div>
                <div style="font-size:.82rem;color:#9298ae">
                    A spreadsheet of every sentence with its category, confidence score, and flags.
                    Open in Excel, Google Sheets, or any data tool.
                </div>
            </div>""", unsafe_allow_html=True)
            csv_bytes = export_csv(st.session_state.df)
            st.download_button("⬇ Download CSV", csv_bytes, "essay_analysis.csv", "text/csv", use_container_width=True)

        with c2:
            st.markdown("""<div class="section-card" style="text-align:center;min-height:160px">
                <div style="font-size:2rem;margin-bottom:.5rem">🗃</div>
                <div style="font-family:'Syne',sans-serif;font-weight:700;margin-bottom:.4rem">JSON</div>
                <div style="font-size:.82rem;color:#9298ae">
                    Full machine-readable dump: discourse units, GoT branches, devil findings, 
                    all scores. Useful for developers or further processing.
                </div>
            </div>""", unsafe_allow_html=True)
            json_bytes = export_json(st.session_state.df, st.session_state.stats,
                                     st.session_state.verdict, st.session_state.subscores,
                                     st.session_state.devil_results, st.session_state.ranked_branches)
            st.download_button("⬇ Download JSON", json_bytes, "essay_analysis.json", "application/json", use_container_width=True)

        with c3:
            st.markdown("""<div class="section-card" style="text-align:center;min-height:160px">
                <div style="font-size:2rem;margin-bottom:.5rem">📄</div>
                <div style="font-family:'Syne',sans-serif;font-weight:700;margin-bottom:.4rem">PDF Report</div>
                <div style="font-size:.82rem;color:#9298ae">
                    A formatted report with verdict, scores, classified sentences, and 
                    devil's advocate findings. Ready to print or submit.
                </div>
            </div>""", unsafe_allow_html=True)
            pdf_bytes = export_pdf_report(st.session_state.df, st.session_state.stats,
                                           st.session_state.verdict, st.session_state.subscores,
                                           st.session_state.keywords, st.session_state.topic,
                                           st.session_state.devil_results, st.session_state.essay_text)
            mime = "application/pdf" if pdf_bytes[:4]==b"%PDF" else "text/plain"
            ext  = "pdf" if mime=="application/pdf" else "txt"
            st.download_button(f"⬇ Download {ext.upper()}", pdf_bytes,
                               f"essay_report.{ext}", mime, use_container_width=True)

        st.markdown("---")
        st.markdown("### 📋 Preview: Classified Sentences")
        st.markdown("""<div class="graph-insight">
            This table shows every sentence from your essay, what category the AI assigned it, 
            how confident it is (0–100%), whether it found a citation, and whether evidence is missing.
        </div>""", unsafe_allow_html=True)
        cols_show = [c for c in ["sentence","category","confidence","has_citation","is_numeric","missing_evidence"]
                     if c in st.session_state.df.columns]
        st.dataframe(st.session_state.df[cols_show], use_container_width=True, height=320)

        col_a,col_b = st.columns(2)
        with col_a:
            st.markdown("**Verdict & Score Summary**")
            st.json({k:v for k,v in st.session_state.verdict.items() if k not in ("strengths","weaknesses","improvements")})
        with col_b:
            st.markdown("**Sub-Scores**")
            st.json(st.session_state.subscores)
